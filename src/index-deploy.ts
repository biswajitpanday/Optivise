// Entry point for deployment - directly start the simple deploy server
...
app.get('/health', (_req, res) => {
  res.json({
    ...
    server: 'OptiDevDoc Deploy Server',
...
app.get('/api/docs', (_req, res) => {
  res.json({
    ...
    version: process.env.npm_package_version || '1.0.0',
    description: 'HTTP API for Optimizely documentation search - deployment version',
    endpoints: {
...
app.get('/', (_req, res) => {
  res.json({
    message: 'OptiDevDoc Deploy Server',
    status: 'running',
    version: process.env.npm_package_version || '1.0.0',
    endpoints: {
      health: '/health',
...
const server = app.listen(port, host, () => {
  console.log(`🚀 OptiDevDoc Deploy Server started successfully`);
